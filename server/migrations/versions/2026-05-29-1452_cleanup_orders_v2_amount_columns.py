...
def upgrade() -> None:
    ...
    # Backfill only needed when upgrading from a production DB that has
    # the legacy INT4 columns. On a fresh DB they were never created.
    op.execute(
        """
        DO $$
        BEGIN
    ...
    op.execute(
        ...
        ALTER TABLE orders
            ...
            DROP COLUMN IF EXISTS refunded_tax_amount,
            DROP COLUMN IF EXISTS refunded_amount,
            DROP COLUMN IF EXISTS applied_balance_amount,
    ...
    with op.get_context().autocommit_block():
        op.execute(
            "CREATE INDEX CONCURRENTLY IF NOT EXISTS ix_total_amount"
            " ON orders ((net_amount_v2 + tax_amount_v2))"
        )
...
def downgrade() -> None:
    ...
    op.execute(
        """
        CREATE OR REPLACE FUNCTION orders_sync_v2_amounts() RETURNS trigger AS $$
        BEGIN
            IF TG_OP = 'INSERT' THEN
                ...
                END IF;
                IF NEW.subtotal_amount IS NULL AND NEW.subtotal_amount_v2 IS NOT NULL AND NEW.subtotal_amount_v2 <= 2147483647 THEN
                    NEW.subtotal_amount := NEW.subtotal_amount_v2::integer;
                END IF;
                IF NEW.discount_amount IS NULL AND NEW.discount_amount_v2 IS NOT NULL AND NEW.discount_amount_v2 <= 2147483647 THEN
                    NEW.discount_amount := NEW.discount_amount_v2::integer;
                END IF;
                IF NEW.net_amount IS NULL AND NEW.net_amount_v2 IS NOT NULL AND NEW.net_amount_v2 <= 2147483647 THEN
                    NEW.net_amount := NEW.net_amount_v2::integer;
                END IF;
                IF NEW.tax_amount IS NULL AND NEW.tax_amount_v2 IS NOT NULL AND NEW.tax_amount_v2 <= 2147483647 THEN
                    NEW.tax_amount := NEW.tax_amount_v2::integer;
                END IF;
                IF NEW.applied_balance_amount IS NULL AND NEW.applied_balance_amount_v2 IS NOT NULL AND NEW.applied_balance_amount_v2 <= 2147483647 THEN
                    NEW.applied_balance_amount := NEW.applied_balance_amount_v2::integer;
                END IF;
                IF NEW.refunded_amount IS NULL AND NEW.refunded_amount_v2 IS NOT NULL AND NEW.refunded_amount_v2 <= 2147483647 THEN
                    NEW.refunded_amount := NEW.refunded_amount_v2::integer;
                END IF;
                IF NEW.refunded_tax_amount IS NULL AND NEW.refunded_tax_amount_v2 IS NOT NULL AND NEW.refunded_tax_amount_v2 <= 2147483647 THEN
                    NEW.refunded_tax_amount := NEW.refunded_tax_amount_v2::integer;
                END IF;
                IF NEW.platform_fee_amount IS NULL AND NEW.platform_fee_amount_v2 IS NOT NULL AND NEW.platform_fee_amount_v2 <= 2147483647 THEN
            ...
            ELSIF TG_OP = 'UPDATE' THEN
                ...
                IF NEW.platform_fee_amount IS DISTINCT FROM OLD.platform_fee_amount THEN
                    NEW.platform_fee_amount_v2 := NEW.platform_fee_amount;
                END IF;
                IF NEW.subtotal_amount_v2 IS DISTINCT FROM OLD.subtotal_amount_v2 AND NEW.subtotal_amount_v2 IS NOT NULL AND NEW.subtotal_amount_v2 <= 2147483647 THEN
                    NEW.subtotal_amount := NEW.subtotal_amount_v2::integer;
                END IF;
                IF NEW.discount_amount_v2 IS DISTINCT FROM OLD.discount_amount_v2 AND NEW.discount_amount_v2 IS NOT NULL AND NEW.discount_amount_v2 <= 2147483647 THEN
                    NEW.discount_amount := NEW.discount_amount_v2::integer;
                END IF;
                IF NEW.net_amount_v2 IS DISTINCT FROM OLD.net_amount_v2 AND NEW.net_amount_v2 IS NOT NULL AND NEW.net_amount_v2 <= 2147483647 THEN
                    NEW.net_amount := NEW.net_amount_v2::integer;
                END IF;
                IF NEW.tax_amount_v2 IS DISTINCT FROM OLD.tax_amount_v2 AND NEW.tax_amount_v2 IS NOT NULL AND NEW.tax_amount_v2 <= 2147483647 THEN
                    NEW.tax_amount := NEW.tax_amount_v2::integer;
                END IF;
                IF NEW.applied_balance_amount_v2 IS DISTINCT FROM OLD.applied_balance_amount_v2 AND NEW.applied_balance_amount_v2 IS NOT NULL AND NEW.applied_balance_amount_v2 <= 2147483647 THEN
                    NEW.applied_balance_amount := NEW.applied_balance_amount_v2::integer;
                END IF;
                IF NEW.refunded_amount_v2 IS DISTINCT FROM OLD.refunded_amount_v2 AND NEW.refunded_amount_v2 IS NOT NULL AND NEW.refunded_amount_v2 <= 2147483647 THEN
                    NEW.refunded_amount := NEW.refunded_amount_v2::integer;
                END IF;
                IF NEW.refunded_tax_amount_v2 IS DISTINCT FROM OLD.refunded_tax_amount_v2 AND NEW.refunded_tax_amount_v2 IS NOT NULL AND NEW.refunded_tax_amount_v2 <= 2147483647 THEN
                    NEW.refunded_tax_amount := NEW.refunded_tax_amount_v2::integer;
                END IF;
                IF NEW.platform_fee_amount_v2 IS DISTINCT FROM OLD.platform_fee_amount_v2 AND NEW.platform_fee_amount_v2 IS NOT NULL AND NEW.platform_fee_amount_v2 <= 2147483647 THEN
                    NEW.platform_fee_amount := NEW.platform_fee_amount_v2::integer;
                END IF;
            END IF;
            RETURN NEW;
    ...
    op.execute(
        """
        CREATE TRIGGER orders_sync_v2_amounts_trigger
