...
# Bootstrap the per-instance database on the shared infra
...
if [[ "${1:-api}" == "api" && ! -f "$BOOTSTRAP_MARKER" ]]; then
    ...
    echo "Bootstrapping per-instance DB '$POLAR_POSTGRES_DATABASE'..."
...
SQL
    ...
    echo "Bootstrap complete: $POLAR_POSTGRES_DATABASE"
fi
